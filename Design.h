/*
    - Plan:
        1. Process Generator:
            ...
            2-  In the main loop that consumes the process queue that is generated after reading the file,
                ...
                It should semaphoreDown a semaphore to make sure that the schedular received the process (when the schedular receives the semaphore, it
                will semaphoreUp the semaphore again).
        ...
        3. The Schedular Algo(ex:HPF,RR):
            ...
            3-  In the while loop,after the dequeue of the process, it will semaphoreDown the semaphore(to be blocked till the process finishes executing)
            4-  When the process finishes,It will send a signal to the schedular algo indicating that it finished.
            5-  The sent signal will semaphoreUp the semaphore to allow the schedular algo to dequeue a new process
    ...
    - IPC:
        ...
        2. Semaphores:
            1-  Between process generator and schedular:The process generator will be blocked at semaphoreDown,till the schedular makes it semaphoreUp after receiving the 
                                                        the process from the message queue and add it to the appropriate data structure of the the schedular algo
            2-  Between schedular algo and itself:The schedular algo will be blocked in semaphoreDown till the process finishes executing.The signal sent
                                                  from the process after finishing executing will make the semaphore semaphoreUp again to allow the schedular algo to
